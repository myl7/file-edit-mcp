...
An MCP (Model Context Protocol) server exposing six file tools — `read`, `write`, `edit`, `multi_edit`, `glob`, `grep` — strictly scoped to the directories given via `--allow`. Every mutating tool enforces the same freshness gate: the file must have been read, and must not have changed since that read, or the call is rejected until it is re-read. There are no shell, process, network, clipboard, or screenshot tools. Two transports: stdio (local, one process per client connection) and streamable HTTP (resident, stateless — it speaks the 2026-07-28 MCP protocol natively, the form ChatGPT custom connectors require, alongside the legacy protocol versions for older clients).
...
- **Read-before-write.** Every mutating tool (`write`/`edit`/`multi_edit`) applies the same freshness gate: you must hold a current read of the target file — a file that has not been read, or that changed since your read, is rejected until re-read (`EStaleRead`; one code, two wordings — never-read and stale) — so a concurrent writer can cause a rejected write but never a silent overwrite. Over stdio the marker scope is the process (one connection); over HTTP the marker scope is the token — every stateless request carrying the same token shares one marker set, so two clients on different tokens cannot unlock writes for each other, while any writer (another token or an out-of-band disk change) stales every other token's marker for that path until it re-reads. Markers reset when the process restarts.
